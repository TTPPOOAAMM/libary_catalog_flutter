...
void main() async {
  ...
  final dio = buildDio(
    ...
    onSessionExpired: () => authNotifier.logout(reason: 'Сессия истекла. Войдите снова.'),
    onForbidden: (msg) {
  ...
  final router = GoRouter(
    ...
    redirect: (context, state) {
      ...
      if (isAuthRoute) return '/books';

      if (path == '/users') {
        if (!AppPermissions.canManageUsers(role)) return '/forbidden';
      }

      if (path == '/loans') {
        if (!AppPermissions.canManageLoans(role)) return '/forbidden';
      }

      if (path == '/readers' || path.startsWith('/readers/')) {
        if (!AppPermissions.canManageReaders(role)) return '/forbidden';
      }

      if (path == '/genres' || path.startsWith('/genres/')) {
        if (!AppPermissions.canManageCatalogs(role)) return '/forbidden';
      }

      if (path == '/publishers' || path.startsWith('/publishers/')) {
        if (!AppPermissions.canManageCatalogs(role)) return '/forbidden';
      }

      if (path == '/books/new' || path.endsWith('/edit')) {
        if (!AppPermissions.canManageBooks(role)) return '/forbidden';
      }
    ...
    routes: [
      ...
      GoRoute(
        ...
        routes: [
          ...
          GoRoute(
            path: ':id',
            builder: (c, s) => BookDetailScreen(bookId: int.tryParse(s.pathParameters['id'] ?? '') ?? 0),
          ),
        ],
      ),
      GoRoute(
        ...
        routes: [
          ...
          GoRoute(path: ':id/edit', builder: (c, s) => AuthorFormScreen(id: int.tryParse(s.pathParameters['id'] ?? ''))),
          GoRoute(
            path: ':id',
            builder: (c, s) => AuthorDetailScreen(authorId: int.tryParse(s.pathParameters['id'] ?? '') ?? 0),
          ),
        ],
      ),
      GoRoute(
        path: '/genres',
        builder: (c, s) => GenreListScreen(queryParams: s.uri.queryParameters),
        routes: [
      ...
      GoRoute(
        ...
        routes: [
          GoRoute(path: 'new', builder: (c, s) => const PublisherFormScreen()),
        ],
      ),
      GoRoute(
        path: '/readers',
